...
def make_valid_request(metjod: str, params: dict | None = None) -> dict:
...
def test_sanity():
    ...
    assert isinstance(sensor_reading_interval, int), "Sensor reading is not a string"

    sensor_reading = get_sensor_reading()
    assert isinstance(
        sensor_reading, float
    ), "Sensor reading doesn't seem to register temperature"
